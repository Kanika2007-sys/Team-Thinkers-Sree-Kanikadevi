...
function getComplaintDetail(c) {
  return ComplaintDetails[c.id] || {
    previousReports: 0, verified: c.trust >= 70, street: c.street || `Ward ${c.ward} Main Road`, nearby: 0,
...
function getCitizenWard() {
  return '18';
}

function getComplaintTitle(c) {
  return c.title || c.category || 'Complaint';
}

function getComplaintSummary(c) {
  return c.description || c.street || `${c.dept} service request`;
}

function normalizeComplaintRecord(c) {
  return {
    ...c,
    title: getComplaintTitle(c),
    description: getComplaintSummary(c),
    labels: Array.isArray(c.labels) ? c.labels : [],
    trust: typeof c.trust === 'number' ? c.trust : 80,
    coords: c.coords || '',
    street: c.street || '',
    previousReports: c.previousReports || 0,
    verified: typeof c.verified === 'boolean' ? c.verified : true,
    nearby: c.nearby || 0,
    ai: c.ai || {},
    evidence: c.evidence || {},
...
function getCitizenComplaints() {
  return AppState.complaints.map(normalizeComplaintRecord).filter(c => String(c.ward) === getCitizenWard());
}

function openCitizenComplaintForm(prefill = {}) {
  const complaint = normalizeComplaintRecord({
    citizen: 'Ward 18 Resident',
    phone: '',
    dept: 'Roads',
    category: '',
    ward: getCitizenWard(),
  ...
  openModal(`
    <div class="modal-head">
      <div>
        <h3 style="font-size:18px; margin:0;">New Complaint</h3>
        <p style="font-size:12px; color:var(--text-faint); margin-top:4px;">Send a service request to your ward office.</p>
      </div>
      <button class="drawer-close" onclick="closeAllOverlays()"><i data-lucide="x" style="width:16px;"></i></button>
    </div>
    <div class="modal-body">
      <form onsubmit="submitCitizenComplaint(event)" style="display:flex; flex-direction:column; gap:14px;">
        <div class="grid-cols-2">
          <div class="form-group"><label>Title</label><input class="form-control" name="title" value="${complaint.title || ''}" placeholder="Short issue summary" required></div>
          <div class="form-group"><label>Department</label><select class="form-control" name="dept"><option ${complaint.dept === 'Roads' ? 'selected' : ''}>Roads</option><option ${complaint.dept === 'Water' ? 'selected' : ''}>Water</option><option ${complaint.dept === 'Sanitation' ? 'selected' : ''}>Sanitation</option><option ${complaint.dept === 'Electricity' ? 'selected' : ''}>Electricity</option><option ${complaint.dept === 'Emergency' ? 'selected' : ''}>Emergency</option></select></div>
        </div>
        <div class="form-group"><label>Description</label><textarea class="form-control" name="description" rows="4" placeholder="Describe what happened, where, and when." required>${complaint.description || ''}</textarea></div>
        <div class="grid-cols-2">
          <div class="form-group"><label>Ward</label><input class="form-control" name="ward" value="${complaint.ward || getCitizenWard()}" inputmode="numeric" required></div>
          <div class="form-group"><label>Priority</label><select class="form-control" name="priority"><option ${complaint.priority === 'Low' ? 'selected' : ''}>Low</option><option ${complaint.priority === 'Medium' ? 'selected' : ''}>Medium</option><option ${complaint.priority === 'High' ? 'selected' : ''}>High</option><option ${complaint.priority === 'Critical' ? 'selected' : ''}>Critical</option></select></div>
        </div>
        <div class="grid-cols-2">
          <div class="form-group"><label>Phone</label><input class="form-control" name="phone" value="${complaint.phone || ''}" placeholder="Contact number"></div>
          <div class="form-group"><label>Location</label><input class="form-control" name="street" value="${complaint.street || ''}" placeholder="Street / landmark"></div>
        </div>
        <div class="flex-gap" style="justify-content:flex-end; margin-top:8px;">
          <button type="button" class="btn btn-outline" onclick="closeAllOverlays()">Cancel</button>
          <button type="submit" class="btn btn-primary"><i data-lucide="send"></i> Submit Complaint</button>
        </div>
      </form>
    </div>
  `);
...
async function submitCitizenComplaint(event) {
  ...
  const payload = {
    ...
    category: form.title.value.trim(),
    ward: form.ward.value.trim(),
    priority: form.priority.value,
    labels: [form.dept.value, 'Citizen Submission'].filter(Boolean),
    trust: 80,
    coords: '',
  ...
  try {
    ...
    const created = normalizeComplaintRecord(await response.json());
    AppState.complaints = [created, ...AppState.complaints.filter(c => c.id !== created.id)];
    closeAllOverlays();
    showToast('Your complaint has been submitted.', 'success', 'send');
    Router.navigate('complaints');
  } catch (error) {
    showToast(error.message || 'Could not submit the complaint.', 'danger');
  }
}
...
const Router = {
  routes: {
    'user-dashboard': renderCitizenDashboard,
...
function renderCitizenDashboard() {
  const citizenWard = '18';
  const citizenName = 'Ward 18 Resident';
  const citizenComplaints = AppState.complaints.filter(c => c.ward === citizenWard);
  const openComplaints = citizenComplaints.filter(c => c.status === 'Pending').length;
  const resolvedComplaints = citizenComplaints.filter(c => c.status === 'Resolved').length;
  const inProgressComplaints = citizenComplaints.filter(c => c.status === 'Assigned' || c.status === 'InProgress').length;
  const pendingComplaints = citizenComplaints.filter(c => c.status === 'Pending').length;

  const categoryCounts = citizenComplaints.reduce((acc, complaint) => {
    acc[complaint.category] = (acc[complaint.category] || 0) + 1;
    return acc;
  }, {});

  const recentComplaints = citizenComplaints.slice().sort((a, b) => b.date.localeCompare(a.date)).slice(0, 3);
  const recentNotifications = AppState.notificationsSent.slice(0, 3);

  return `
    <div class="glass-card" style="margin-bottom:24px; background:linear-gradient(135deg, var(--bg-card) 0%, rgba(37, 99, 235, 0.05) 100%); border-color:var(--border);">
      <div class="flex-between" style="gap:24px; flex-wrap:wrap;">
        <div>
          <span class="badge success" style="margin-bottom:12px;"><span class="dot"></span> Verified Citizen Profile</span>
          <h1 style="font-size:32px; margin-bottom:8px; font-family:var(--font-disp);">Welcome, ${citizenName}!</h1>
          <p style="color:var(--text-muted); font-size:15px; max-width:700px;">Track your complaints, review alerts, and manage your citizen profile from one place.</p>
        </div>
        <div class="flex-center" style="width:80px; height:80px; border-radius:50%; background:linear-gradient(135deg, var(--brand) 0%, var(--brand-hover) 100%); color:#fff; font-size:24px; font-weight:800;">WR</div>
      </div>
    </div>

    <div class="grid-cols-4" style="margin-bottom:32px;">
      <div class="glass-card"><div class="flex-between" style="margin-bottom:16px;"><h4 style="color:var(--text-muted); font-size:13px; text-transform:uppercase;">Total Complaints</h4><i data-lucide="file-text" style="width:16px;height:16px;color:var(--info);"></i></div><h2 style="font-size:36px; margin-bottom:8px;">${citizenComplaints.length}</h2><span style="font-size:11px; color:var(--text-faint);">Your submissions</span></div>
      <div class="glass-card"><div class="flex-between" style="margin-bottom:16px;"><h4 style="color:var(--text-muted); font-size:13px; text-transform:uppercase;">Open Complaints</h4><i data-lucide="clock" style="width:16px;height:16px;color:var(--warning);"></i></div><h2 style="font-size:36px; margin-bottom:8px;">${openComplaints}</h2><span style="font-size:11px; color:var(--text-faint);">Pending review</span></div>
      <div class="glass-card"><div class="flex-between" style="margin-bottom:16px;"><h4 style="color:var(--text-muted); font-size:13px; text-transform:uppercase;">In Progress</h4><i data-lucide="refresh-cw" style="width:16px;height:16px;color:var(--brand);"></i></div><h2 style="font-size:36px; margin-bottom:8px;">${inProgressComplaints}</h2><span style="font-size:11px; color:var(--text-faint);">Assigned to crew</span></div>
      <div class="glass-card"><div class="flex-between" style="margin-bottom:16px;"><h4 style="color:var(--text-muted); font-size:13px; text-transform:uppercase;">Resolved Tickets</h4><i data-lucide="check-circle" style="width:16px;height:16px;color:var(--success);"></i></div><h2 style="font-size:36px; margin-bottom:8px;">${resolvedComplaints}</h2><span style="font-size:11px; color:var(--text-faint);">Verified solutions</span></div>
    </div>

    <div class="grid-cols-2" style="margin-bottom:32px;">
      <div class="glass-card" style="display:flex; flex-direction:column; gap:24px;">
        <div>
          <h3 style="font-size:18px; margin-bottom:4px;">My Service Analytics</h3>
          <p style="font-size:13px; color:var(--text-muted);">Status breakdown and categories distribution.</p>
        </div>
        ${citizenComplaints.length === 0 ? `<div style="min-height:220px; display:flex; align-items:center; justify-content:center; color:var(--text-faint);">No complaints submitted yet.</div>` : `
          <div style="display:flex; flex-direction:column; gap:20px;">
            <div>
              <h4 style="font-size:13px; color:var(--text-muted); margin-bottom:10px;">My Complaint Status</h4>
              <div style="height:24px; background:var(--border-light); border-radius:12px; overflow:hidden; display:flex;">
                ${pendingComplaints > 0 ? `<div style="width:${(pendingComplaints / citizenComplaints.length) * 100}%; background:var(--warning); color:#fff; font-size:10px; font-weight:700;" class="flex-center">${Math.round((pendingComplaints / citizenComplaints.length) * 100)}%</div>` : ''}
                ${inProgressComplaints > 0 ? `<div style="width:${(inProgressComplaints / citizenComplaints.length) * 100}%; background:var(--brand); color:#fff; font-size:10px; font-weight:700;" class="flex-center">${Math.round((inProgressComplaints / citizenComplaints.length) * 100)}%</div>` : ''}
                ${resolvedComplaints > 0 ? `<div style="width:${(resolvedComplaints / citizenComplaints.length) * 100}%; background:var(--success); color:#fff; font-size:10px; font-weight:700;" class="flex-center">${Math.round((resolvedComplaints / citizenComplaints.length) * 100)}%</div>` : ''}
              </div>
            </div>
            <div>
              <h4 style="font-size:13px; color:var(--text-muted); margin-bottom:12px;">Category Distribution</h4>
              <div style="display:flex; flex-direction:column; gap:10px;">
                ${Object.entries(categoryCounts).map(([category, count], i) => `
                  <div style="font-size:13px;">
                    <div class="flex-between" style="margin-bottom:4px;"><strong>${category}</strong><span class="text-muted">${count} complaint${count > 1 ? 's' : ''}</span></div>
                    <div style="height:6px; background:var(--border-light); border-radius:3px; overflow:hidden;"><div style="height:100%; width:${(count / citizenComplaints.length) * 100}%; background:hsl(${(i * 70) % 360}, 70%, 55%);"></div></div>
                  </div>
                `).join('')}
              </div>
            </div>
      ...
      <div class="glass-card" style="padding:0;">
        <div style="padding:24px; border-bottom:1px solid var(--border);"><h3 style="font-size:18px;">Recent Complaints</h3><p style="font-size:13px; color:var(--text-muted); margin-top:4px;">Latest ticket activity in your ward.</p></div>
        <div style="padding:24px; display:flex; flex-direction:column; gap:14px;">
          ${recentComplaints.length === 0 ? `<div style="text-align:center; padding:24px 0; color:var(--text-faint);">No recent complaints to show.</div>` : recentComplaints.map(c => `
            <div style="padding:14px; border:1px solid var(--border); border-radius:var(--radius-md); background:var(--bg-page);">
              <div class="flex-between" style="margin-bottom:6px;"><strong>${c.id}</strong>${getStatusBadge(c.status)}</div>
              <p style="font-size:13px; color:var(--text-muted); margin-bottom:4px;">${c.category} · ${c.dept}</p>
              <p style="font-size:12px; color:var(--text-faint);">Ward ${c.ward} · ${c.date}</p>
            </div>
          `).join('')}
        </div>
      </div>
    ...
    <div class="grid-cols-2" style="margin-bottom:24px;">
      <div class="glass-card" style="padding:0;">
        <div style="padding:24px; border-bottom:1px solid var(--border);"><h3 style="font-size:18px;">Recent Notifications</h3></div>
        <div style="padding:24px; display:flex; flex-direction:column; gap:12px;">
          ${recentNotifications.map(n => `<div style="padding:12px; border:1px solid var(--border); border-radius:var(--radius-md); background:var(--bg-page);"><strong style="display:block; margin-bottom:4px;">${n.title}</strong><p style="font-size:12.5px; color:var(--text-muted); line-height:1.4;">${n.type} · ${n.area} · ${n.priority}</p></div>`).join('')}
        </div>
      </div>
      <div class="glass-card" style="padding:0;">
        <div style="padding:24px; border-bottom:1px solid var(--border);"><h3 style="font-size:18px;">Emergency Alerts</h3></div>
        <div style="padding:24px; display:flex; flex-direction:column; gap:12px;">
          <div style="padding:12px; border:1px solid var(--border); border-radius:var(--radius-md); background:var(--bg-page);"><strong style="display:block; margin-bottom:4px;">Flood Watch</strong><p style="font-size:12.5px; color:var(--text-muted); line-height:1.4;">Monitor low-lying areas and follow evacuation guidance.</p></div>
          <div style="padding:12px; border:1px solid var(--border); border-radius:var(--radius-md); background:var(--bg-page);"><strong style="display:block; margin-bottom:4px;">Power Interruptions</strong><p style="font-size:12.5px; color:var(--text-muted); line-height:1.4;">Temporary outages may affect the east sector during maintenance.</p></div>
        </div>
      </div>
    </div>

    <div class="grid-cols-3">
      <div class="glass-card" style="padding:20px;"><h3 style="font-size:18px; margin-bottom:12px;">Quick Actions</h3><div style="display:flex; flex-direction:column; gap:10px;"><button class="btn btn-primary" onclick="openCitizenComplaintForm()">New Complaint</button><button class="btn btn-outline" onclick="Router.navigate('complaints')">My Complaints</button><button class="btn btn-outline" onclick="Router.navigate('notifications')">Notifications</button></div></div>
      <div class="glass-card" style="padding:20px;"><h3 style="font-size:18px; margin-bottom:12px;">Profile Summary</h3><div style="display:flex; flex-direction:column; gap:8px; font-size:13px;"><div class="flex-between"><span class="text-muted">Ward</span><strong>${citizenWard}</strong></div><div class="flex-between"><span class="text-muted">Trust Score</span><strong>92/100</strong></div><div class="flex-between"><span class="text-muted">Member Since</span><strong>April 2026</strong></div></div></div>
      <div class="glass-card" style="padding:20px;"><h3 style="font-size:18px; margin-bottom:12px;">Contact Support</h3><p style="font-size:13px; color:var(--text-muted); line-height:1.5;">Use the complaint form to report service issues or contact the ward office for urgent follow-up.</p></div>
    </div>
  `;
}
...
function getFilteredComplaints() {
  ...
  let list = getCitizenComplaints().filter(c => {
    if (f.dept !== 'All' && c.dept !== f.dept) return false;
    if (f.priority !== 'All' && c.priority !== f.priority) return false;
    if (f.status !== 'All' && c.status !== f.status) return false;
    if (f.search) {
      const q = f.search.toLowerCase();
      const hay = `${c.id} ${c.title} ${c.description} ${c.citizen} ${c.category} ${c.ward}`.toLowerCase();
...
function renderComplaintRows(list) {
  if (list.length === 0) {
    return `<tr><td colspan="6" style="text-align:center; padding:48px; color:var(--text-faint);">No complaints match these filters.</td></tr>`;
  }
  return list.map(c => `
    <tr class="fade-in" style="cursor:pointer;" onclick="viewComplaint('${c.id}')">
      ...
      <td>
        <span style="font-weight:600; display:block;">${getComplaintTitle(c)}</span>
        <span style="font-size:11px; color:var(--text-muted);">${getComplaintSummary(c)}</span>
      </td>
      <td>
        <span style="font-weight:500; display:block;">${c.dept}</span>
...
function viewComplaint(id) {
  const c = normalizeComplaintRecord(AppState.complaints.find(x => x.id === id) || {});
  if (!c.id) return;
    ...
    <div class="drawer-head">
      <div>
        <div style="display:flex; align-items:center; gap:8px;">
          <h3 style="font-size:18px; margin:0;">${getComplaintTitle(c)}</h3>
          ${getStatusBadge(c.status)}
        </div>
        <p style="font-size:12px; color:var(--text-muted); margin-top:4px;">${c.dept} · Ward ${c.ward}</p>
      </div>
      <button class="drawer-close" onclick="closeAllOverlays()"><i data-lucide="x" style="width:16px;"></i></button>
    </div>
    <div class="drawer-body">
      <div class="glass-card" style="padding:16px; margin-bottom:20px;">
        <h4 style="font-size:13px; text-transform:uppercase; color:var(--text-muted); margin-bottom:10px;">Complaint Details</h4>
        <div class="flex-between" style="margin-bottom:10px;"><span>Title</span><strong>${getComplaintTitle(c)}</strong></div>
        <div class="flex-between" style="margin-bottom:10px;"><span>Description</span><strong style="text-align:right; max-width:65%;">${getComplaintSummary(c)}</strong></div>
        <div class="flex-between" style="margin-bottom:10px;"><span>Priority</span><strong style="color:${getPriorityColor(c.priority)};">${c.priority}</strong></div>
        <div class="flex-between" style="margin-bottom:10px;"><span>Submitted</span><strong>${c.date}</strong></div>
        <div class="flex-between" style="margin-bottom:10px;"><span>Location</span><strong>${c.street || `Ward ${c.ward} Main Road`}</strong></div>
        <div class="flex-between"><span>Labels</span><strong>${c.labels.length ? c.labels.join(', ') : 'None'}</strong></div>
      </div>

      <div class="glass-card" style="padding:16px; margin-bottom:20px;">
        <h4 style="font-size:13px; text-transform:uppercase; color:var(--text-muted); margin-bottom:10px;">Updates</h4>
      ...
      <div class="flex-gap" style="margin-top:8px;">
        <button class="btn btn-primary" style="flex:1;" onclick="openCitizenComplaintForm({ title: getComplaintTitle(c), description: getComplaintSummary(c), ward: c.ward, dept: c.dept, street: c.street })"><i data-lucide="plus"></i> Follow Up</button>
        <button class="btn btn-outline" style="flex:1;" onclick="closeAllOverlays()">Close</button>
      </div>
    </div>
  `);
}
...
function renderCitizenComplaints() {
  ...
  return `
    <div class="flex-between" style="margin-bottom: 24px; gap:16px; flex-wrap:wrap;">
      <div>
        <h2 style="font-size:22px; margin:0;">My Complaints</h2>
        <p style="font-size:13px; color:var(--text-muted); margin-top:4px;">Only your ward complaints and service requests are shown here.</p>
      </div>
      <div class="flex-gap">
        <button class="btn btn-outline" style="font-size:12px; padding: 6px 12px;" onclick="openCitizenComplaintForm()"><i data-lucide="plus"></i> New Complaint</button>
      </div>
    </div>
      ...
      <div class="flex-between" style="flex-wrap:wrap; gap:12px;">
        <div class="flex-gap" style="flex-wrap:wrap;">
          <div class="search-container" style="width:220px;">
            ...
            <input type="text" placeholder="Search title, ward..." style="height:34px; font-size:12px;" oninput="updateComplaintFilter('search', this.value)">
          </div>
        ...
        <span class="badge info" style="font-size:12px;" id="complaints-count-badge">${list.length} of ${list.length}</span>
      </div>
    </div>

    <div class="grid-cols-4" style="margin-bottom:24px;">
      <div class="glass-card" style="padding:18px;"><div class="text-muted" style="font-size:12px; text-transform:uppercase; margin-bottom:6px;">Total</div><h2 style="margin:0;">${list.length}</h2></div>
      <div class="glass-card" style="padding:18px;"><div class="text-muted" style="font-size:12px; text-transform:uppercase; margin-bottom:6px;">Pending</div><h2 style="margin:0;">${list.filter(c => c.status === 'Pending').length}</h2></div>
      <div class="glass-card" style="padding:18px;"><div class="text-muted" style="font-size:12px; text-transform:uppercase; margin-bottom:6px;">In Progress</div><h2 style="margin:0;">${list.filter(c => c.status === 'Assigned' || c.status === 'InProgress').length}</h2></div>
      <div class="glass-card" style="padding:18px;"><div class="text-muted" style="font-size:12px; text-transform:uppercase; margin-bottom:6px;">Resolved</div><h2 style="margin:0;">${list.filter(c => c.status === 'Resolved').length}</h2></div>
    </div>
    ...
    <div class="glass-card" style="padding:0; overflow:hidden;">
      <div class="table-container">
        <table>
          ...
          <tbody id="complaints-tbody">${renderComplaintRows(list)}</tbody>
        </table>
      </div>
    </div>
  `;
...
function renderComplaints() {
  return renderCitizenComplaints();
}
